...
- Graph load: `kg/load_graph.py` loaded all 47 entities and all 50
  relationships from the Part 1 seed spreadsheet with zero skipped rows —
...
- Historical accuracy of each fact was already checked against its cited
  Wikipedia source during Part 1 seeding — this part's validation is about
...
- Entities get a second Neo4j label matching `entity_type`
  (`:Person`/`:Location`/`:Event`/`:Organization`/`:Cause`) so Part 4's
...
- The 3 `:Concept` nodes are candidates for promotion to real entities if
  Part 4 retrieval needs to traverse through them (e.g. "rebel sepoys" ->
