...
- Pipeline: `generation/generate_answer.py` chains Week 4's
...
- Ran all 10 questions in `data/eval/sample_questions.csv` through the full
  ...
  [docs/week5_sample_answers.md](week5_sample_answers.md).
...
- **Q05 ("Who commanded the British forces that recaptured Delhi?") —
  ...
  limitation already flagged in `docs/week4_notes.md`'s open items). With
  ...
  didn't say "I don't know" when it should have — worth watching in Week
  6's trust scoring and Week 7's evaluation set.
- **Q08 ("What happened to Bahadur Shah II after the fall of Delhi?") —
  ...
  subtle, source-grounded-but-still-wrong error type that Week 6's
...
of case Week 6 (trust scoring) and Week 7 (ablation) are specifically
built to catch and quantify, not bugs to silently patch over. Recommend
adding both Q05 and Q08's underlying question types to the Week 7 30-40
...
- Consider 2-hop graph expansion (or expanding seed entities via one
  ...
  after seeing how often this pattern recurs in Week 7's larger test set.
- Week 6's trust score should be run against Q08's transcript as a sanity
